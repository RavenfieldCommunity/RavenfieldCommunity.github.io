...
4. 不安装, 使用原版（可能影响观感，除非其他选项无法使用否则不建议）"
  ...
  else { Get-MLinkBinary "$global:gamePath/$global:fontName" $downloadUrls }
}

function Set-TConfig {
  Write-Host "正在应用配置 ..."
...
" | Set-Content -Path $configPath
    ...
    $result_ = (Get-Content -Path $configPath) -Replace '\[Service]', "[Service]
Endpoint=
" | Set-Content -Path $configPath
  }
  else {
    ...
    $result_ = New-Item -Path $targetDirectory -Name "AutoTranslatorConfig.ini" -ItemType "file" -Value "[Service]
Endpoint=
[Behaviour]
OverrideFontTextMeshPro=$global:fontName"
  }
...
function Get-Translator {
  if ( (Test-Path -Path "$global:gamePath\BepInEx\core\XUnity.Common.dll") -eq $true ) {
    ...
    $result_ = Get-MLinkBinary $translatorDownloadPath ($translatorUrls)
    if ($result_ -ne $false) {
...
function Get-MLang {
  ...
  if ( (Test-Path -Path $file1) -eq $true ) {
    Write-Host "已经订阅翻译文件"
    if ( (Test-Path -Path $targetDirectoryPath) -ne $true ) { 
      #如果目标目录不存在则新建
      $result_ = mkdir $targetDirectoryPath;
    } 
    #如果目录创建成功
    if ($? -eq $true) {
      #1
...
if ($yesRun -eq "2") { 
  ...
  $global:redirectSrc = Get-MLinkString 'http://ravenfieldcommunity.github.io/static/uninstall_bepinex-utf8.ps1', 'https://gitee.com/RavenfieldCommunity/UnionSetup/raw/master/uninstall_bepinex-utf8.txt', 'https://ravenfieldcommunity-static.netlify.app/uninstall_bepinex-utf8.ps1'
	
  if ($null -eq $global:redirectSrc) {
    Write-Warning "重定向失败";
    Exit-IScript
  }
  else { Invoke-Expression $global:redirectSrc; }
}
