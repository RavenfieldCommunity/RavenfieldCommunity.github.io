...
Class VdfTextReader {
  ...
  hidden [bool]EnsureBuffer() {
    ...
    #A bit of mathgic to improve performance by avoiding a conditional.
    $this._charBuffer[0] = $this._charBuffer[($this._charsLen - 1) * $remainingChars]; 
...
function Get-MLinkBinary([string]$path, [string[]]$links) {
...
function Get-MLangInfo ([string]$cn, [string]$en) {
  if ((Get-Culture).Name -eq "zh-CN") {
    return $true;
  }
  else { 
...
function Exit-IScript {
  Write-MLangOutput "您现在可以关闭窗口了" "Now you can close this window";
  Read-Host;
  Exit;
  Exit-IScript;
}
...
if ( $global:downloadPath.Contains("AppData") -ne $true) {
  #有些玩家的设备真的逆天
  $global:downloadPath = "D:\Temp\RavenfieldCommunityCN\";
}
#如果下载路径不存在则新建
if ( (Test-Path -Path $downloadPath) -ne $true) {
  $result_ = mkdir $downloadPath;
} 
#测试与打印下载目录
Write-MLangOutput "下载目录: $downloadPath" "Download path: $downloadPath";
...
function Get-BepInEXGithub {
  ...
  if ( (Test-Path -Path "$global:gamePath\winhttp.dll") -eq $true ) {
    ...
    if ($? -eq $true) {
      ...
      Expand-Archive -Path $bepInEXDownloadPath -DestinationPath $global:gamePath -Force;
      if ($? -eq $true) {
...
function Get-BepInEXCN {
  ...
  #创建session并请求文件
  $result = Get-MLinkBinary $bepInEXDownloadPath $bepInEXUrls;
  if ($result -eq $false) {
    #错误处理
